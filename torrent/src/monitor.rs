...
";
    ...
    // ── check_completion edge cases ─────────────────────────

    #[test]
    fn test_check_completion_empty_phases() {
        let checkpoint = Checkpoint {
            id: "arc-empty".into(),
            schema_version: Some(24),
            plan_file: "plan.md".into(),
    ...
    #[test]
    fn test_check_completion_merge_takes_priority_over_failed() {
        // If merge completed but another phase failed, merge wins
        let mut phases = std::collections::HashMap::new();
        phases.insert("work".into(), crate::checkpoint::PhaseStatus {
            status: "failed".into(),
            started_at: None,
        ...
        match check_completion(&checkpoint) {
            Some(ArcCompletion::Merged) => {}
            other => panic!("expected Merged (priority over Failed), got {other:?}"),
        }
    }

    #[test]
    fn test_check_completion_ship_completed_merge_in_progress() {
        // ship done + merge in_progress = not terminal (merge still running)
        let mut phases = std::collections::HashMap::new();
        phases.insert("ship".into(), crate::checkpoint::PhaseStatus {
            status: "completed".into(),
            started_at: None,
        ...
        assert!(check_completion(&checkpoint).is_none(),
            "ship+in_progress merge should NOT be terminal");
    }

    #[test]
    fn test_check_completion_all_phases_pending() {
        // Full pipeline, everything pending — not terminal
        let mut phases = std::collections::HashMap::new();
        for &name in PHASE_ORDER {
            phases.insert(name.into(), crate::checkpoint::PhaseStatus {
                status: "pending".into(),
                started_at: None,
                completed_at: None,
                team_name: None,
    ...
    #[test]
    fn test_check_completion_merge_failed() {
        // merge: failed = terminal (Failed, not Merged)
        let mut phases = std::collections::HashMap::new();
        phases.insert("ship".into(), crate::checkpoint::PhaseStatus {
            status: "completed".into(),
            started_at: None,
    ...
    #[test]
    fn test_check_completion_ship_pending_merge_skipped() {
        // ship pending + merge skipped = not terminal (ship hasn't run yet)
        let mut phases = std::collections::HashMap::new();
        phases.insert("ship".into(), crate::checkpoint::PhaseStatus {
            status: "pending".into(),
            started_at: None,
        ...
        assert!(check_completion(&checkpoint).is_none(),
            "pending ship should not be terminal even if merge is skipped");
    }

    #[test]
    fn test_check_completion_with_empty_status_strings() {
        // Phases with empty status strings = not terminal
        let mut phases = std::collections::HashMap::new();
        phases.insert("ship".into(), crate::checkpoint::PhaseStatus {
            status: String::new(),
            started_at: None,
    ...
    #[test]
    fn test_phase_navigation_transitioning_between_phases() {
        // forge completed, work pending, nothing in_progress = transitioning
        let mut phases = std::collections::HashMap::new();
        phases.insert("forge".into(), crate::checkpoint::PhaseStatus {
            status: "completed".into(),
        ...
        phases.insert("plan_review".into(), crate::checkpoint::PhaseStatus {
            status: "pending".into(),
            started_at: None,
            completed_at: None,
            team_name: None,
        ...
        let nav = compute_phase_navigation(&checkpoint).unwrap();
        // prev = forge (last completed)
        assert_eq!(nav.prev.as_ref().unwrap().name, "forge");
        // current = None (no in_progress phase)
        assert!(nav.current.is_none());
        // next = plan_review (first pending after forge)
        assert_eq!(nav.next, Some("plan_review".into()));
    }

    #[test]
    fn test_phase_navigation_with_skipped_phases() {
        // forge completed, plan_review skipped, work in_progress
        let mut phases = std::collections::HashMap::new();
        phases.insert("forge".into(), crate::checkpoint::PhaseStatus {
            status: "completed".into(),
            started_at: Some("2026-03-20T00:00:00Z".into()),
        ...
        phases.insert("plan_refine".into(), crate::checkpoint::PhaseStatus {
            status: "skipped".into(),
            started_at: None,
            completed_at: None,
            team_name: None,
        ...
        phases.insert("work".into(), crate::checkpoint::PhaseStatus {
            status: "in_progress".into(),
            started_at: Some("2026-03-20T00:10:00Z".into()),
            completed_at: None,
            team_name: None,
        });
        phases.insert("ship".into(), crate::checkpoint::PhaseStatus {
    ...
    // ── Phase summary edge cases ────────────────────────────

    #[test]
    fn test_compute_phase_summary_with_skipped() {
        let mut phases = std::collections::HashMap::new();
        phases.insert("forge".into(), crate::checkpoint::PhaseStatus {
            status: "completed".into(),
            started_at: None,
        ...
        phases.insert("design_extraction".into(), crate::checkpoint::PhaseStatus {
            status: "skipped".into(),
            started_at: None,
            completed_at: None,
            team_name: None,
        });
        phases.insert("design_prototype".into(), crate::checkpoint::PhaseStatus {
            status: "skipped".into(),
            started_at: None,
            completed_at: None,
            team_name: None,
        ...
        assert_eq!(summary.current_phase_name, "work");
    }

    #[test]
    fn test_compute_phase_summary_all_completed() {
        let mut phases = std::collections::HashMap::new();
        phases.insert("forge".into(), crate::checkpoint::PhaseStatus {
            status: "completed".into(),
            started_at: None,
