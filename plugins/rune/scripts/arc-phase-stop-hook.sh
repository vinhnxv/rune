...
# <!-- v3.x: arc.persistence.enabled defaults to false in v3.x; see references/v3-defaults.md -->
# Persistence retry path was talisman-gated (arc.persistence.enabled) and is disabled by
# default. With the talisman shard layer removed in v3.x, the gate always evaluates false
# and this entire block is dead code. Removed to simplify the stop hook.
