#!/bin/bash
# PostToolUse hook: detects writes to .rune/echoes/ (or legacy .claude/echoes/) and writes dirty signal
...
if [[ "$FILE_PATH" == *".rune/echoes/"*"MEMORY.md" ]] || \
   [[ "$FILE_PATH" == *".claude/echoes/"*"MEMORY.md" ]]; then
