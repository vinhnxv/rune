...
| **arc** | End-to-end orchestration pipeline (pre-flight freshness gate + 40 phases: forge → plan review → plan refinement → verification → semantic verification → design extraction → design prototype → task decomposition → work → storybook verification → design verification → ux verification → gap analysis → codex gap analysis → gap remediation → goldmask verification → code review (--deep) → goldmask correlation → mend → verify mend → design iteration → test → test coverage critique → pre-ship validation → release quality check → ship → bot review wait → PR comment resolution → merge) |
...
| `figma-context` | `get_figma_data`, `download_figma_images` | AI-optimized Figma data extraction via figma-context-mcp (Framelink). ~90% data compression vs raw Figma API. Fallback provider for design-sync when Rune's figma-to-react MCP is unavailable. Requires `FIGMA_TOKEN`. Launched via `npx -y figma-developer-mcp@0.8.0`. |
