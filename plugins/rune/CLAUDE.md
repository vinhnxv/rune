...
| `PostToolUse:Write\|Edit` | `scripts/verify-arc-state-integrity.sh` | ARC-STATE-INTEGRITY-001: After any Write/Edit to `.rune/arc/*/checkpoint.json`, verifies phase-loop state file exists and auto-creates via `rune-arc-init-state.sh` in hook source mode. Dry-run mode when `arc.state_file.code_enforced_writes=false` (logs observations without touching filesystem). Fast-path grep exit (<1ms) for non-checkpoint writes. R16 re-entrancy guard via `RUNE_ARC_STATE_INIT_IN_PROGRESS`. Closes plan AC-1/AC-7. OPERATIONAL — fail-forward, always exit 0. |
