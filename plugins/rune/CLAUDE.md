...
| `Stop` | `scripts/detect-workflow-complete.sh` | CDX-7: Hook-driven workflow boundary cleanup (Layer 5). Detects arc pipeline completion via `tmp/arc-result-current.json` signal file. Triggers deterministic teammate shutdown and team cleanup at workflow boundary. Runs BEFORE on-session-stop.sh. |
