...
| `_StopFailure_disabled` | `scripts/on-stop-failure.sh` | ⚠ ORPHAN (T5/HOOK-001, audit 20260414-012408): `StopFailure` is not a valid Claude Code hook event, so this script never fires. Entry renamed to `_StopFailure_disabled` and script header updated. API-error checkpoint preservation is NOT active until migrated to `Stop` (with branch detection) or `PostToolUseFailure`. Uses `scripts/lib/stop-failure-common.sh` (v2.59.0, currently unreachable — ships in preparation for the migration; classifies stop failures as rate-limit / auth / server / network with backoff prompt construction). |
