...
| `Stop` | `scripts/context-percent-stop-guard.sh` | CTX-STOP-001: Context usage warning at Stop events. Reads statusline bridge file (`/tmp/rune-ctx-{SESSION_ID}.json`) — same source as `guard-context-critical.sh`. Warning tier (70% used): advisory + `exit 2`. High tier (85% used): strong advisory + `exit 2`. Max 2 blocks per session. Never fires on `context_limit` or user-abort stops. Exits silently when arc loop state files detected. Placed LAST in Stop array. Fail-forward (OPERATIONAL). Talisman-gated: `context_stop_guard.*`. |
