...
| `PostToolUse:Write\|Edit` | `scripts/verify-arc-state-integrity.sh` | ARC-STATE-INTEGRITY-001: On Write/Edit of `.rune/arc/*/checkpoint.json`, verifies phase-loop state file exists and auto-creates via `rune-arc-init-state.sh`. Dry-run when `arc.state_file.code_enforced_writes=false` (does NOT create/modify arc state files; writes observation entries tagged dry_run: true to `.rune/arc-integrity-log.jsonl`). Closes plan AC-1/AC-2/AC-7. OPERATIONAL — fail-forward. |
