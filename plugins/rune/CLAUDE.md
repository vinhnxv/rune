...
| `PreToolUse:Bash` | `scripts/enforce-rtk.sh` | RTK-001: Optional RTK (Rust Token Killer) output compression. Rewrites eligible Bash commands with `rtk` prefix to reduce verbose CLI token consumption. Disabled by default — opt-in via `rtk.enabled: true` in talisman. Two-layer exemption: Layer 2 (command patterns: git blame/diff/log, test runners, linters) checked before Layer 1 (workflows: goldmask, mend, inspect, debug). Compound commands and heredocs are not rewritten. Fail-forward (OPERATIONAL). |
