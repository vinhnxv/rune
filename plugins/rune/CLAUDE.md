...
| **arc** | End-to-end orchestration pipeline (pre-flight freshness gate + 34 phases: forge → plan review → plan refinement → verification → semantic verification → design extraction → design prototype → task decomposition → work → drift review → storybook verification → design verification → ux verification → gap analysis → codex gap analysis → gap remediation → goldmask verification → code review (--deep) → goldmask correlation → mend → verify mend → design iteration → test → browser test → browser test fix → verify browser test → test coverage critique → deploy verify → pre-ship validation → release quality check → ship → bot review wait → PR comment resolution → merge) |
