...
| **design-prototype** | Standalone Figma-to-Storybook prototype generator — 5-phase pipeline (extract → match → synthesize → verify → present). Gated by `design_sync.enabled`. Two input modes: Figma URL (full pipeline) or text description (library search only). Non-invocable knowledge + user-invocable slash command |
