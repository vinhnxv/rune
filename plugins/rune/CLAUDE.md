...
| `PostToolUse:SendMessage` | `scripts/track-teammate-activity.sh` | TEAMMATE-ACTIVITY-001: Per-teammate liveness detection. Tracks last-activity timestamps for teammates and flags stuck agents exceeding `process_management.teammate_stuck_threshold` (default 180s). OPERATIONAL — fail-forward. |
...
- [Agent registry](references/agent-registry.md) — 142 agent definitions (99 CORE in agents/ + 43 EXTENDED in registry/, 13 shared). 12 stack specialist reviewers are prompt templates, not registered agents
