...
9. **Polling loop fidelity**: When translating `waitForCompletion` pseudocode, you MUST call the `TaskList` tool on every poll cycle — not just sleep and hope. The correct sequence per cycle is: `TaskList()` → count completed → check stale/timeout → `Bash("sleep 30", { run_in_background: true })` → repeat. Derive loop parameters from config — not arbitrary values: `maxIterations = ceil(timeoutMs / pollIntervalMs)` and `sleep $(pollIntervalMs / 1000)`. See monitor-utility.md per-command configuration table for exact values.
   - If you are not waiting on a multi-teammate Team, see `polling-guard/SKILL.md` § "Choose the Right Waiting Pattern" — `Monitor` and background `Bash` exist for non-Team waits and MUST NOT be replaced with `TaskList` loops. Concrete `Monitor` recipes live in `polling-guard/references/monitor-tool-patterns.md`.
...
17. **Iron Law ARC-QA-002 — Stop Hook Self-Heal Precedence (PLANNED, not yet shipped)**: The stop hook SHOULD check for late-arriving artifacts before retrying any `in_progress` phase. The self-heal helper at `scripts/lib/arc-phase-self-heal.sh` + `_phase_find_next()` wiring is planned per plan `plans/2026-04-19-fix-arc-qa-notification-race-plan.md` AC-4 (SHOULD priority), deferred from v2.58.0 to a follow-up release. **Until it ships**, automatic recovery from `in_progress` phases with on-disk artifacts relies entirely on Iron Law ARC-QA-001 (LLM-mediated 3-check) — operators who see a stuck phase should inspect `tmp/arc/{id}/qa/*-verdict.json` and `tmp/arc/{id}/.done/` directly and flip status manually if needed.
...
| `_StopFailure_disabled` | `scripts/on-stop-failure.sh` | ⚠ ORPHAN (T5/HOOK-001, audit 20260414-012408): `StopFailure` is not a valid Claude Code hook event, so this script never fires. Entry renamed to `_StopFailure_disabled` and script header updated. API-error checkpoint preservation is NOT active until migrated to `Stop` (with branch detection) or `PostToolUseFailure`. |
