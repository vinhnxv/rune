...
> **Prefix note**: When embedded in Pattern Weaver Ash, use the `QUAL-` finding prefix per the dedup hierarchy (`SEC > BACK > VEIL > DOUBT > FLOW > DOC > QUAL > FRONT > CDX`). The standalone prefix `REFAC-` is used only when invoked directly.
...
> **Note**: When embedded in Pattern Weaver Ash, replace `REFAC-` prefix with `QUAL-` in all finding IDs per the dedup hierarchy (`SEC > BACK > VEIL > DOUBT > FLOW > DOC > QUAL > FRONT > CDX`). The `REFAC-` prefix below is used in standalone mode only.
