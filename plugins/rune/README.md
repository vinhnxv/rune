...
- **Concurrent sessions** — Multiple Claude Code sessions can run simultaneously on the same directory. Supported: reader + writer (e.g., audit while arc runs), planner + writer, reader + reader. Writer + writer conflicts are blocked by workflow locks. Each session's hooks are scoped to their own session via 3-layer isolation (config_dir + session_id + owner_pid). A non-Rune session (plain chat) will NOT interfere with an active arc pipeline.
...
| Another tab hijacks my arc pipeline | Fixed in v1.153.3. Update plugin and restart Claude Code. Set `RUNE_TRACE=1` to verify session isolation in `/tmp/rune-hook-trace-*.log` |
