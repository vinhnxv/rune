...
This is the **Rune plugin** — the detailed component reference for the Rune multi-agent orchestration system. It documents all 144 agents, 59 skills, 16 commands, 4 MCP servers, and the hook infrastructure that powers Rune's workflows.
...
**Want the full pipeline?** Run `/rune:arc plans/my-plan.md` for an automated 43-phase pipeline: plan enrichment → code review → auto-fix → testing → PR → merge.
...
| Arc | End-to-end pipeline | 43-phase automated workflow |
...
Chains 43 phases: Forge (plan enrichment) → Plan Review → Work → Gap Analysis → Code Review → Mend (auto-fix findings) → Test → Ship → Merge. Checkpoint-based resume (`--resume`) available if interrupted.
