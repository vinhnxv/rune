...
| 34 | `blind_verification` | Post-strive AC-only verification | `enabled`, `blocking`, `timeout_seconds` |
...
## Plugin Monitors (`process_management.monitors.enabled`)

Single kill-switch for all declared plugin monitors in `monitors/monitors.json`.
Part of Track B of the Monitor tool integration (plan: Rune Monitor Tool Integration Strategy, 2026-04-18).
...
process_management:
  monitors:
    enabled: true   # default — plugin monitors auto-start at SessionStart when host is capable
```

Setting `process_management.monitors.enabled: false` disables ALL declared plugin monitors at SessionStart.
Rune falls back to existing Stop-hook / PostToolUse-hook coverage automatically (e.g.,
`detect-stale-lead.sh` stops backing off and re-engages its 4-method detection cascade).

### Semantics

| Setting | Behavior |
|---------|----------|
| `true` (default) | Declared monitors start at SessionStart when the AC-5 probe writes `tmp/.rune-monitor-available`. On unsupported hosts (Bedrock/Vertex/Foundry/DISABLE_TELEMETRY/CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC) monitors are skipped silently. |
| `false` | No plugin monitors start, regardless of host capability. Stop-hook fallback paths handle what the monitors would have covered. |

### Edge cases

- **Mid-session flip does NOT terminate running monitors** (EC-6). A flip to `false` takes
  effect at the *next* SessionStart. If you need to kill a running monitor immediately,
  stop the monitor process directly or start a new session.
- **Per-monitor scoping is enforced in declarations, not in talisman**. `stale-teammate-watcher`
  fast-path-exits when no workflow signals exist. `echo-search-eager-reindex` uses
  `when: "on-skill-invoke:rune-echoes"` to bound lifetime. No per-monitor on/off flag exists
  in the talisman schema.
- **Claude Code version floor**: plugin monitors require Claude Code v2.1.105+. On older
  versions, the probe writes `tmp/.rune-monitor-unavailable` and the kill-switch is moot.

### References

- `monitors/monitors.json` — declared monitor descriptors
- `scripts/probe-monitor-availability.sh` — AC-5 host capability probe
- `skills/polling-guard/references/monitor-tool-patterns.md` — companion script patterns
- `skills/arc/references/arc-phase-bot-review-wait.md` — Track A concrete Monitor consumer
