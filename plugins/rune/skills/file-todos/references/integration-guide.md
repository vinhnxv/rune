...
**Migration note (CC-4)**: If you previously used the v2.x talisman file_todos.dir key, remove that key — todos are now session-scoped and no project-root override is supported. If you used the `archive` subcommand, completed session data is now available in Rune Echoes (`.rune/echoes/`) and git history.
