...
The Figma screenshot is the PRIMARY validation source. `get_design_context()` and
...
2. **Metadata SECOND** — `figma_fetch_design()` or `get_metadata()` for structure overview
3. **Design context THIRD** — `get_design_context()` or `figma_inspect_node()` for tokens
