...
Scans feature description, selected approach, and round transcripts for Figma URLs. Validates with SSRF blocklist, previews components via `figma_list_components` when `design_sync.enabled`, and injects design context into advisor rounds.

See [design-asset-detection.md](references/design-asset-detection.md) for the full implementation code including SSRF defense and component preview logic.
