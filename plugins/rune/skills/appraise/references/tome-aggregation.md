...
**Important**: After Phase 5.5 Cross-Model Verification, HALLUCINATED and UNVERIFIED findings are **removed** from the TOME before Runebinder produces the final output. Only CONFIRMED findings from Codex Oracle are included.

If inscription.json has `verification.enabled: true`:

### TOME.md Structure

The TOME (Total Output of Multiple Eyes) follows this canonical structure:
