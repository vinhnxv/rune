...
// 5c. Monitor tool — streaming path (default-on when host-capable) — AC-2 / Track A.1
//
// Reads the AC-5 sentinel written by probe-monitor-availability.sh. When the host
// supports the Monitor tool, replace the pull-based polling loop below with a
// push-based streaming subprocess that emits `STATE_CHANGE` lines only when the
// bot+CI signature actually changes. The underlying `gh api` cadence is
// UNCHANGED — the win is Claude-side context economy (~85% fewer messages),
// not GitHub-side load reduction.
//
// Capability gate (AC-5 host probe):
//   tmp/.rune-monitor-available   → use Monitor (default path)
//   tmp/.rune-monitor-unavailable → skip Monitor, use polling (fallback — see below)
//   (no sentinel)                 → skip Monitor, use polling (conservative)
//
// Runtime circuit breaker:
//   2 consecutive STATE_ERROR emissions → abort Monitor, fall back to polling,
//   write tmp/arc/{id}/monitor-fallback.log with reason.
//
// Fallback state does NOT persist across phase invocations (RISK-C). Each arc
// run re-attempts Monitor from scratch.
//
// No opt-in config flag exists for this path — capability probe + runtime circuit
// breaker gate activation (Q1 resolution: capability-driven, default-on).
//
// The polling loop immediately below remains code-resident (RISK-D) —
// NEVER comment it out or delete it. It is the fallback path.

let monitorOutcome = null  // "streamed" | "fallback" | "unavailable"
let monitorActivityTimestamp = new Date().toISOString()
let monitorDetectedBots = new Set()

const monitorSentinelOk = `${CLAUDE_PROJECT_DIR ?? "."}/tmp/.rune-monitor-available`
const monitorSentinelNo = `${CLAUDE_PROJECT_DIR ?? "."}/tmp/.rune-monitor-unavailable`
const monitorAvailable = Bash(`test -f "${monitorSentinelOk}" && echo yes || echo no`).trim() === "yes"

if (monitorAvailable) {
  log(`Monitor tool available — streaming bot/CI state changes (circuit breaker active, 2 consecutive STATE_ERROR → fallback).`)

  // Injection-safe: pass owner/repo/prNumber/headSha via env map, never string-interp into heredoc.
  // `headSha` mirrors the polling-loop variable. The Monitor runs for up to HARD_TIMEOUT_MS.
  // Emission rule: only on state-change signature delta (BP-1) OR consecutive failure (EC-2).
  // Baseline emission: the first cycle emits STATE_CHANGE even without a prior signature (BP-3
  // LIST-then-WATCH) so the orchestrator has positive evidence of stream liveness.
  try {
    const reviewMonitor = Monitor({
      ...
      env: {
        ...
        BOT_PATTERN: botLoginPattern
      },
      command: `
        # STATE_CHANGE emits only when bot-or-CI signature changes.
        # STATE_ERROR emits once per consecutive failure — 2 in a row triggers
        # the orchestrator-side circuit breaker and fallback to polling.
        # Baseline emission on first cycle (BP-3 LIST-then-WATCH).
        last_signature=""
        fail_streak=0
        first_emit=1
        while true; do
    ...
    // Consume the stream. Two consecutive STATE_ERROR = circuit breaker fires.
    let consecutiveErrors = 0
    let eventCount = 0
    for (const event of reviewMonitor) {
      const line = (event.output ?? event.stdout ?? "").toString().trim()
      if (!line) continue
      eventCount += 1
      if (line.includes("STATE_ERROR")) {
        consecutiveErrors += 1
        warn(`bot-review-wait Monitor: STATE_ERROR ${consecutiveErrors}/2 — ${line}`)
        if (consecutiveErrors >= 2) {
          warn("bot-review-wait Monitor: circuit breaker tripped — falling back to polling.")
          Write(`tmp/arc/${id}/monitor-fallback.log`, `Monitor circuit breaker tripped after 2 consecutive STATE_ERROR events.\nLast line: ${line}\nTimestamp: ${new Date().toISOString()}\n`)
          monitorOutcome = "fallback"
          break
        }
        continue
      }
      if (line.includes("STATE_CHANGE")) {
        consecutiveErrors = 0
        monitorActivityTimestamp = new Date().toISOString()
        // RISK-B: assert steady-state event budget. >10 unique events per arc run = signature noise bug.
        if (eventCount > 10) {
          warn(`bot-review-wait Monitor: emitted ${eventCount} events — possible signature noise (non-deterministic field leaked?). Investigate.`)
        }
      }
    }

    if (monitorOutcome !== "fallback") {
      // Stream ended cleanly (timeout hit, or PR review finished and exit 0).
      monitorOutcome = "streamed"
      log(`bot-review-wait Monitor: stream ended after ${eventCount} events.`)
    }
  } catch (monitorErr) {
    // Any throw from Monitor invocation → fallback (RISK-C: same behaviour as circuit breaker)
    warn(`bot-review-wait Monitor: invocation failed (${monitorErr.message}) — falling back to polling.`)
    Write(`tmp/arc/${id}/monitor-fallback.log`, `Monitor invocation threw: ${monitorErr.message}\nTimestamp: ${new Date().toISOString()}\n`)
    monitorOutcome = "fallback"
  }
} else {
  monitorOutcome = "unavailable"
  log("Monitor tool unavailable on this host (Bedrock/Vertex/Foundry/telemetry-disabled) — using polling fallback.")
}

// If Monitor streamed to completion, skip the polling loop.
// Otherwise (fallback or unavailable), the polling loop below takes over from scratch —
// polling does NOT inherit Monitor's last_signature (EC-4). This is safe because
// polling has always been the historical sole implementation.
const skipPollingLoop = monitorOutcome === "streamed"

// 6. Multi-signal polling loop (FALLBACK — required; do NOT delete)
// This block is reached when:
//   - monitorOutcome === "fallback"    (circuit breaker tripped)
//   - monitorOutcome === "unavailable" (host probe negative)
//   - monitorOutcome === null          (probe missing — conservative path)
//
...
if (skipPollingLoop) {
  // Monitor handled this phase in full — skip the pull loop.
  log("bot-review-wait: Monitor stream completed. Skipping polling fallback.")
}
