...
function generateQADashboard(arcId) {
  ...
  // Cross-validation: flag suspicious score patterns
  const warnings = []
  for (const r of scoredPhases) {
    // Inflated artifact score: high artifact but low quality suggests shallow verification
    if (r.artifact_score >= 95 && r.quality_score < 70) {
      warnings.push(`${r.phase}: inflated artifact score (artifact=${r.artifact_score}, quality=${r.quality_score})`)
    }
    // Orchestrator fallback: timed-out verdicts are infrastructure failures, not quality signals
    if (r.timed_out) {
      warnings.push(`${r.phase}: verdict generated by orchestrator (timed_out=true) *`)
    }
  }
  // Suspiciously uniform: all phases within 2 points suggests mechanical scoring
  if (scoredPhases.length >= 3) {
    const scores = scoredPhases.map(r => r.overall_score)
    const minScore = Math.min(...scores)
    const maxScore = Math.max(...scores)
    if (maxScore - minScore <= 2) {
      warnings.push(`All ${scoredPhases.length} phases scored within 2 points (${minScore}-${maxScore}) — suspiciously uniform`)
    }
  }
...
function generateDashboardMarkdown(summary) {
  ...
  for (const r of summary.phase_results) {
    if (r.verdict === "SKIPPED") {
      ...
      const timedOutStr = r.timed_out ? " *" : ""
      md += `| ${r.phase} | ${r.verdict}${retryStr}${timedOutStr} | ${r.overall_score} | ${r.artifact_score} | ${r.quality_score} | ${r.completeness_score} | ${checkStr} |\n`
    }
  }

  // Cross-validation warnings
  if (summary.cross_validation_warnings && summary.cross_validation_warnings.length > 0) {
    md += `\n**Cross-validation warnings:**\n`
