...
// CORRECT: Call TaskList on every poll cycle. See monitor-utility.md and the POLL-001 hook (enforce-polling.sh).
...
if (!cleanupTeamDeleteSucceeded) {
  ...
  // CLEAN-001 (audit 20260427-032216): Replaced inline `for pid in $(pgrep -P $PPID)`
  // with canonical _rune_kill_tree which applies full MCP-PROTECT-003 classification
  // (40+ MCP binaries, --stdio/--lsp/--sse transport markers, connector patterns).
  // The prior inline pattern only filtered --stdio, leaving SSE-transport MCP servers
  // (e.g., @upstash/context7-mcp) and --lsp LSP servers vulnerable.
  Bash(`source "${RUNE_PLUGIN_ROOT}/scripts/lib/process-tree.sh" && _rune_kill_tree "$PPID" "2stage" "5" "teammates" "arc-plan-review-${id}"`)
...
## Phase 2.5 (absorbed): CONCERN Extraction & Escalation

<!-- v3.0.0-alpha.6: absorbed from the deleted arc-phase-plan-refine.md. -->
<!-- Same plan_review phase keeps the verdicts side-by-side with the CONCERN -->
<!-- context extraction — no need for a separate orchestrator-only phase. -->

After the reviewer team has been shut down and TeamDelete completed (above), still inside the **`plan_review` phase**, run the CONCERN extraction + escalation sub-step. This used to be a separate `plan_refine` orchestrator-only phase but adds no value as a phase boundary — it just reads the verdict files plan_review already wrote.

Runs ONLY if at least one reviewer raised CONCERN. If all PASS, nothing to do here — plan_review's checkpoint update at the top of this file already marked the phase complete.

```javascript
// ── Sub-step 2.5: CONCERN extraction (orchestrator-only, no team) ──

// Canonical reviewers — must match the spawn list above.
const reviewers = [
...
// parseVerdict — inline so this sub-step is self-contained.
const parseVerdict = (reviewer, output) => {
  const pattern = /^<!-- VERDICT:([a-zA-Z_-]+):(PASS|CONCERN|BLOCK) -->$/m
  const match = output.match(pattern)
  if (!match) { warn(`Reviewer ${reviewer} output lacks verdict marker — defaulting to CONCERN.`); return "CONCERN" }
...
// STEP 2: If no CONCERNs, nothing to do — plan_review is already complete.
...
if (allConcern) {
  ...
  if (!checkpoint.flags.confirm) {
    ...
    if (escalationResponse.includes("Halt")) {
      ...
      // CROSS-PHASE CHECKPOINT DEMOTION: Reset plan_review to "pending".
      // On --resume, the dispatcher will re-run Phase 2 from scratch.
      updateCheckpoint({
    ...
    // "Proceed with warnings" — fall through.
  }
}
```

**Output**: `tmp/arc/{id}/concern-context.md` (absent when no CONCERNs surfaced).

**Failure policy**: Non-blocking — proceed with unrefined plan + deferred concerns as context.

**Cross-phase mutation**: When the user selects "Re-run plan review" during all-CONCERN escalation, this sub-step demotes `plan_review` itself back to `"pending"` so `--resume` will re-execute Phase 2. This is the only place in the arc pipeline that mutates another phase's checkpoint state — preserved verbatim from the absorbed plan_refine phase.
