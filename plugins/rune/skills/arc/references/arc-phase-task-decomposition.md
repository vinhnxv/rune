...
5. Run: "${RUNE_PLUGIN_ROOT}/scripts/codex-exec.sh" -m "${codexModel}" -r "${reasoning}" -t ${timeout} -j -g -o tmp/arc/${id}/task-validation.md tmp/arc/${id}/.codex-prompt-task-decomp.tmp
...
10. Mark task complete`
  ...
  for (let attempt = 0; attempt < TD_CLEANUP_DELAYS.length; attempt++) {
    if (attempt > 0) Bash(`sleep ${TD_CLEANUP_DELAYS[attempt] / 1000}`, { run_in_background: true })
