...
- **Polling enforcement hook**: `enforce-polling.sh` (POLL-001) blocks `sleep+echo` anti-patterns at runtime during active Rune workflows. The hook header comment documents the correct monitoring loop pattern. Together with text warnings in skill bodies, this forms a 2-layer enforcement pyramid (hook + text warnings) that prevents the LLM from improvising sleep-based monitoring proxies.
