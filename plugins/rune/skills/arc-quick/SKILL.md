...
A simplified alternative to `/rune:arc` (40 phases) for small-to-medium features.
...
if (isComplex && !force) {
  AskUserQuestion({
    ...
    options: [
      { label: "Switch to /rune:arc (full 40-phase pipeline)",
...
- \`/rune:arc ${planPath}\` --- run full 40-phase pipeline if needed
