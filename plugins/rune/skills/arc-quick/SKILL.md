...
A simplified alternative to `/rune:arc` (29 phases) for small-to-medium features.
...
if (isComplex && !force) {
  AskUserQuestion({
    ...
    options: [
      { label: "Switch to /rune:arc (full 29-phase pipeline)",
...
- \`/rune:arc ${planPath}\` --- run full 29-phase pipeline if needed
