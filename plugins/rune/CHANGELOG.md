...
- **`cli-correction-detector.sh`** — Detects error→success sequences within a sliding window (default: 5). Classifies 6 error types (UnknownFlag, CommandNotFound, WrongPath, WrongSyntax, PermissionDenied, Timeout), scores confidence (base 0.5 + same-tool/similar-args/multi-session bonuses), deduplicates with Jaccard word-overlap
