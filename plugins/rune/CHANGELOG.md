...
### Fixed
- `on-session-stop.sh`: Extract session_id from hook input JSON instead of unavailable env vars (AC-4)
- `suggest-self-audit.sh`: Guard all 4 arc loop types (phase, batch, hierarchy, issues), not just phase loop (AC-5)
