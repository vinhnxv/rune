...
## rtk — Rust Token Killer Bash command rewriter (v1.129.0+)
...
RTK exemption uses two independent layers. A command is exempt if EITHER matches:

- **Layer 2 — Command-level** (always checked first): command matches any `exempt_commands` pattern
- **Layer 1 — Workflow-level**: active Rune workflow matches any `exempt_workflows` entry

When concurrent state files exist (e.g., both a filter and an exempt workflow), **exempt wins**.

### Known Limitations

- **Compound commands** (`&&`, `||`, `;`, `|`): not rewritten — partial wrapping produces inconsistent behavior. Pass through unchanged.
- **Heredoc commands** (`<< MARKER`): not rewritten. Herestrings (`<<<`) are safe and may be rewritten.
- **Env var prefixes** (`RUST_BACKTRACE=1 cargo test`): env vars are extracted and placed before the RTK prefix in the rewritten command.
- **Already-prefixed commands**: `rtk cargo build` is not double-prefixed.
