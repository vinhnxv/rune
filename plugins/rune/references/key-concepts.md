...
- Runs the unified inspect engine between work and code review (deterministic pre-checks + 4-Inspector audit + halt-gate + gap-fixer dispatch + convergence; v3.0.0-alpha.7 Day 6 absorbed the former gap_analysis + gap_remediation phases)
...
End-to-end orchestration (16 phases as of v3.0.0-alpha.7): forge (research enrichment) → forge_qa → plan_review (3-reviewer circuit breaker, with absorbed CONCERN-extraction post-step formerly `plan_refine`) → verification gate (deterministic checks, zero-LLM) → work (swarm implementation, with absorbed drift-signal review formerly `drift_review`) → work_qa → inspect (unified plan-vs-implementation engine — STEP A deterministic pre-checks formerly `gap_analysis`'s STEP A, 4 Inspector Ashes verdict pass, STEP D halt-gate with Task Completion Gate + plan writeback formerly `gap_analysis`'s STEP D, gap-fixer dispatch formerly `gap_remediation`, and convergence eval formerly `verify_inspect` — all one phase with `inspect_convergence` retry loop) → code_review (Roundtable Circle, deep) → code_review_qa → verify (TOME finding classification) → mend (parallel finding resolution) → mend_qa (1-agent QA scoring + absorbed convergence-gate post-step formerly `verify_mend` — smart scoring, scope-aware signals, adaptive retry cycles based on tier) → test (diff-scoped unit/property/integration/E2E execution) → test_qa → ship (preShipValidator dual-gate completion check formerly `pre_ship_validation` + auto PR creation via `gh pr create`; `deploy_verify` removed entirely in v3.x) → merge (rebase + squash-merge with pre-merge checklist).
