...
- Nếu bạn mới dùng Rune: đọc hết tài liệu này theo thứ tự từ trên xuống.
- Nếu bạn đã cài xong: bắt đầu ngay ở phần `Plan -> Work -> Review`.
- Nếu cần lệnh copy-paste nhanh: xem [Quick Cheat Sheet](rune-quick-cheat-sheet.vi.md).
- Nếu có câu hỏi thường gặp: xem [FAQ Rune](rune-faq.vi.md).
- Nếu gặp thuật ngữ khó: xem [Thuật ngữ Rune (Tiếng Việt)](rune-glossary.vi.md).
- Mục tiêu tối thiểu cho ngày đầu:
  1. Tạo 1 plan bằng `/rune:plan`
  2. Triển khai bằng `/rune:work`
  3. Review bằng `/rune:review`
...
Rune v3.x dùng giá trị mặc định cố định — không cần bước cấu hình thủ công. Xem [`plugins/rune/references/v3-defaults.md`](../../plugins/rune/references/v3-defaults.md) để biết danh mục giá trị mặc định.
...
Khi đã quen với workflow cơ bản, hãy khám phá các lệnh nâng cao:
...
- [Hướng dẫn Arc và batch](rune-arc-and-batch-guide.vi.md) — End-to-end pipeline
...
Có! Dùng `/rune:test-browser` cho test E2E trình duyệt độc lập. Chạy workflow 9 bước inline mà không cần agent teams.
